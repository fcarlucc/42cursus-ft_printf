...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/21 20:44:41 by fcarlucc          #+#    #+#             */
/*   Updated: 2023/05/21 22:06:46 by fcarlucc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	select_dot_type_z(va_list param, const char *str_format, t_flag *flag)
{
	...
	if (str_format[flag->index] == 's')
		return (flag_zero_dot_s(va_arg(param, char *), flag));
	else if (str_format[flag->index] == 'd' || str_format[flag->index] == 'i')
		return (flag_zero_dot_d(va_arg(param, int), flag));
	else if (str_format[flag->index] == 'u')
		return (flag_zero_dot_u(va_arg(param, unsigned int), flag));
	else if (str_format[flag->index] == 'p')
		return (flag_zero_dot_p(va_arg(param, unsigned long), flag));
	else if (str_format[flag->index] == 'x')
		return (flag_zero_dot_x(va_arg(param, unsigned int), flag));
	else if (str_format[flag->index] == 'X')
		return (flag_zero_dot_xx(va_arg(param, unsigned int), flag));
	else
		return (manage_flag_zero(param, str_format, flag));
}
